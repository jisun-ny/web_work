...
<%@ page language="java" contentType="application/octet-stream; charset=UTF-8"
    ...
    <!-- 파일 데이터를 응답할거니까 contentType="text/html을 octet-stream으로 바꿔준다. -->
<%
	...
	*/	
	   ...
	   FileDto dto=FileDao.getInstance().getData(num); //우리는 이미 Dao에 파일 번호만 넘겨주면 파일 정보를 리턴해주는 메소드를 만들어 뒀다
    ...
    //다운로드 시켜줄 파일명 인코딩 //안하면 한글이 깨지니까
    String encodedName=URLEncoder.encode(orgFileName, "utf-8");
	 //파일명에 공백이있는 경우 처리 //역슬래쉬들을 공백으로 잘 바꿔야함 그래야 오류가 안남.
    ...
    // HttpServletReponse 객체에 있는 OutputStream이 바이트 알갱이를 출력할 수 있게 해줌. 
    // response -> HttpServletReponse type // response.getOutputStream() -> OutputStream (바이트 알갱이를 출력할 수 있는 객체)
