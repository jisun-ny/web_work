...
<body>
	<div class="container">
		...
		<p>내 이름은 <strong>${myName }</strong> </p> <%--pageScope는 생략가능. 알아서 페이지 스코프 영역 찾고 없으면 리퀘스트영역 찾고.. key값만 잘 적으면 됨! --%>
	</div>
	...
	<div class="container">
		...
		<%--setAttibute한 것을 getAttribute하고 캐스팅까지 해야 사용할 수 있는 문자열을 EL을 사용하면 간단하게 불러올 수 있다. --%>
		
		<h1>EL로 session scope에 저장된 값 추출</h1>
		<p>우리의 이름은 <strong>${sessionScope.ourName }</strong> </p>
		<p>우리의 이름은 <strong>${ourName }</strong> </p>
		<%--위의 EL은 아래의 코드를 대신 할 수 있다 --%>
		<%
			String result2 = (String)session.getAttribute("ourName");
		%>
		<p>우리의 이름은 <strong><%=result2 %></strong></p>
		
		<h1>EL로 applicaion scope에 저장된 값 추출</h1>
		<p>학원의 이름은 <strong>${applicationScope.companyName }</strong> </p>
		<p>학원의 이름은 <strong>${companyName }</strong> </p>
	</div>
</body>
</html>